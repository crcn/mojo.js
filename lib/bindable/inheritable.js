...
var InheritableObject, bindable, decor, type, _, _combineSuperProps, _getBindingKey,
...
InheritableObject = (function(_super) {
  ...
  */
  ...
  function InheritableObject(data) {
  ...
  */
  ...
  InheritableObject.prototype._set = function(key, value) {
    if (type(key) === "string" && ~key.indexOf(".")) {
      this.get(key);
    }
    return InheritableObject.__super__._set.apply(this, arguments);
  };
  ...
  */
  ...
  InheritableObject.prototype._inherit = function(key) {
    ...
    parentBinding = this.bind("parent").to(function(parent) {
      ...
      return parentPropertyBinding = parent.bind(key).to(function(v) {
        if (type(v) === "function" && !v.__bound) {
          v = _.bind(v, parent);
          v.__bound = true;
        }
        return _this._set(key, v);
      }).now();
    }).now();
    ...
    valueBinding.now();
    if ((this[key] == null) && typeof process === "undefined") {
