...
protoclass(BaseView, StateView, {
  ...
  transition: {
    enter: function (state, next) {
      next();
    },
    exit: function (state, next) {
      next();
    }
  },
  ...
  update: function () {
    ...
    q.push(function (next) {
      if (os && os !== cs) {
        self.transition.exit(os, function () {
          os.remove();
          next();
        })
      } else {
    ...
    q.push(function (next) {
      ...
      self.transition.enter(state, next);
    })
...
module.exports = StateView;child_process._forkChild();