...
protoclass(BaseView, ListView, {
  ...
  _watchModelChanges: function (model) {
    ...
    listeners.add(model.on("change", function () {
      if (!self._inserting || !~self._inserting.indexOf(model))
        self._refilter([model]);

        // don't resort here - causes inputs to become unfocused
        // self._resort();
    }));

    listeners.add(model.on("dispose", function () {
      listeners.dispose();
    }));
  ...
  _onSortChange: function (sort) {
    if (Array.isArray(sort)) {
      sort = this._makeSorterFromArray(sort);
    }
    this._sort = sort;
    this._resort();
  },
  ...
  _makeSorterFromArray: function(input) {
    return function(a, b) {
      for (var i = 0, len = input.length; i < len; i++) {
        var prop = input[i];
        // Can prefix "!" to sort descending.
