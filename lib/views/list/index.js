...
protoclass(BaseView, ListView, {
  ...
  _render: function () {
    ...
    if (this.modelViewClass) {
      this._modelViewFactory = this.application.getPool(this.modelViewClass) || this._modelViewFactory;
    }
  ...
  _insertNow: function (models, resort) {
    ...
    for (var i = 0, n = models.length; i < n; i++) {
      ...
      if(~this._views.searchIndex({ _id: model.__context._id })) continue;
      ...
      views.push(view);
      frags.push(view.render().toFragment());
    }
  ...
  _onRemoveModel: function (model, index, viewIndex) {
    ...
    if (viewIndex === undefined) {
      viewIndex = this._views.searchIndex({ _id: model.__context._id });
    }
  ...
  _refilter: function (models) {
    ...
    for (i = models.length; i--;) {
      ...
      modelIndex  = this._views.searchIndex({ _id: model.__context._id });
