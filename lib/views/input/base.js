// Generated by CoffeeScript 1.6.2
(function() {
  var __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
    __hasProp = {}.hasOwnProperty,
    __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; };

  define(["../base", "../../templates/factory"], function(BaseView, templates) {
    var BaseInputView, _ref;

    return BaseInputView = (function(_super) {
      __extends(BaseInputView, _super);

      function BaseInputView() {
        this._onValueChange = __bind(this._onValueChange, this);        _ref = BaseInputView.__super__.constructor.apply(this, arguments);
        return _ref;
      }

      /*
      */


      BaseInputView.prototype.events = {
        "input input": "_onInputChange"
      };

      /*
      */


      BaseInputView.prototype._onRendered = function() {
        BaseInputView.__super__._onRendered.call(this);
        return this.bind("value", this._onValueChange);
      };

      BaseInputView.prototype.changed = function() {
        return this.emit("data", {
          name: this.get("name"),
          value: this.get("value")
        });
      };

      /*
       Reflect the value change in the text input
      */


      BaseInputView.prototype._onValueChange = function(value) {
        return this.changed();
      };

      return BaseInputView;

    })(BaseView);
  });

}).call(this);
