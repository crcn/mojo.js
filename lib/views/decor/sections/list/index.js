...
(function() {
  ...
  define(["../../../collection", "../../../adapters/index", "dref", "hoist", "pilot-block", "bindable", "type-component", "../../../../factories/fn", "../../../../factories/class"], function(ViewCollection, adapters, dref, hoist, pilot, bindable, type, FnFactory, ClassFactory) {
    var ListSection;
    ...
    ListSection = (function(_super) {
      ...
      */
      ...
      ListSection.prototype._onSourceChange = function(source) {
        ...
        this._sourceBinding.filter(function(model) {
          ...
          return _this.options.filter(model);
        });
        return binding.map(this._modelTransformer).to(this._viewCollection).now();
      };
      ...
      */
      ...
      ListSection.prototype._watchModelChanges = function(model) {
        var filter, onChange, removeListener,
          _this = this;

        if (!this.options.filter) {
          return;
        }
        filter = this.options.filter(model);
        ...
        return model.on("change", onChange = function() {
          ...
          newFilter = _this.options.filter(model);
          if (filter === newFilter) {
            return;
          }
          filter = newFilter;
          if (filter) {
            return _this._viewCollection.push(_this._modelTransformer(model));
          } else {
            return _this._viewCollection.splice(_this._viewCollection.indexOf({
              _id: model.get("_id")
            }), 1);
          }
        });
